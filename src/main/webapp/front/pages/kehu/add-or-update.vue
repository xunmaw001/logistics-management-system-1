<template>
	<view class="content">
		<form class="app-update-pv">
			...
			<view :style='{"padding":"0","boxShadow":"0 0 0px rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"rgba(65, 65, 65, 1)","margin":"0 0 20rpx 0","borderRadius":"0","borderWidth":"0 0 4rpx","width":"100%","borderStyle":"solid","height":"108rpx"}' class="cu-form-group" @tap="touxiangTap">
				...
				<view class="right-input" style="padding:0">
					<image :style='{"padding":"0","boxShadow":"0 0 0px rgba(0,0,0,.3)","margin":"0","borderColor":"#ccc","backgroundColor":"rgba(0,0,0,0)","borderRadius":"100%","borderWidth":"4rpx","width":"88rpx","borderStyle":"solid","height":"88rpx"}' class="avator" v-if="ruleForm.touxiang" :src="baseUrl+ruleForm.touxiang" mode="aspectFill"></image>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`kehu`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(this.ruleForm.kehushouji&&(!this.$validate.isMobile(this.ruleForm.kehushouji))){
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`kehu`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`kehu`, this.ruleForm);
						}else{
							await this.$api.add(`kehu`, this.ruleForm);
						}
