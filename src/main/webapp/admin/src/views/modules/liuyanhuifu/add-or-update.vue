    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="反馈回复" prop="fankuihuifu">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.fankuihuifu" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.fankuihuifu" label="反馈回复" prop="fankuihuifu">
                    <span v-html="ruleForm.fankuihuifu"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	userid : false,
      ...
      rules: {
          ...
          fankuihuifu: [
                { required: true, message: '反馈回复不能为空', trigger: 'blur' },
          ],
          tupian: [
  ...
  created() {
	this.ruleForm.huifushijian = this.getCurDate()
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.biaoti = true;
          ...
          if(o=='fankuihuifu'){
            this.ruleForm.fankuihuifu = obj[o];
	    this.ro.fankuihuifu = true;
            continue;
          }
          if(o=='tupian'){
	...
	this.ruleForm.fankuihuifu = this.ruleForm.fankuihuifu.replace(reg,'../../../ssms772z/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `liuyanhuifu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.liuyanhuifuCrossAddOrUpdateFlag = false;
						 this.parent.search();
