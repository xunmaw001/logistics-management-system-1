<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="物流单号" prop="wuliudanhao">
          <el-input v-model="ruleForm.wuliudanhao" 
              placeholder="物流单号" clearable  :readonly="ro.wuliudanhao"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="物流单号" prop="wuliudanhao">
              <el-input v-model="ruleForm.wuliudanhao" 
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="签收时间" prop="qianshoushijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.qianshoushijian" 
                type="datetime"
                :readonly="ro.qianshoushijian"
                placeholder="签收时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.qianshoushijian" label="签收时间" prop="qianshoushijian">
              <el-input v-model="ruleForm.qianshoushijian" 
                placeholder="签收时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  created() {
	this.ruleForm.qianshoushijian = this.getCurDateTime()
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.yuangonggonghao = true;
          ...
          if(o=='qianshoushijian'){
            this.ruleForm.qianshoushijian = obj[o];
	    this.ro.qianshoushijian = true;
            continue;
          }
          if(o=='userid'){
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `peisongwancheng/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.peisongwanchengCrossAddOrUpdateFlag = false;
						 this.parent.search();
