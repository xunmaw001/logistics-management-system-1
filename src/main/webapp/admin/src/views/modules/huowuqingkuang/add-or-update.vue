<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="收件时间" prop="shoujianshijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.shoujianshijian" 
                type="datetime"
                :readonly="ro.shoujianshijian"
                placeholder="收件时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.shoujianshijian" label="收件时间" prop="shoujianshijian">
              <el-input v-model="ruleForm.shoujianshijian" 
                placeholder="收件时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  created() {
	this.ruleForm.shoujianshijian = this.getCurDateTime()
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `huowuqingkuang/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.huowuqingkuangCrossAddOrUpdateFlag = false;
						 this.parent.search();
