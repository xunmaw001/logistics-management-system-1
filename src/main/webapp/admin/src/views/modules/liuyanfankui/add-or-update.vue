<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="留言日期" prop="liuyanriqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.liuyanriqi" 
                type="date"
                :readonly="ro.liuyanriqi"
                placeholder="留言日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.liuyanriqi" label="留言日期" prop="liuyanriqi">
              <el-input v-model="ruleForm.liuyanriqi" 
                placeholder="留言日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="客户手机" prop="kehushouji">
          <el-input v-model="ruleForm.kehushouji" 
              placeholder="客户手机" clearable  :readonly="ro.kehushouji"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="客户手机" prop="kehushouji">
              <el-input v-model="ruleForm.kehushouji" 
                placeholder="客户手机" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  created() {
	this.ruleForm.liuyanriqi = this.getCurDate()
  ...
  methods: {
    ...
    init(id,type) {
      ...
      this.$http({
		...
		if(json.kehushouji!=''&&json.kehushouji){
	...
	this.ruleForm.liuyanneirong = this.ruleForm.liuyanneirong.replace(reg,'../../../ssms772z/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `liuyanfankui/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.liuyanfankuiCrossAddOrUpdateFlag = false;
						 this.parent.search();
